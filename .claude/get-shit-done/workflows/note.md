...
- **Global scope**: `C:/Korner flag/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Korner flag/.claude/notes/`)
...
2. Glob `C:/Korner flag/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Korner flag/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Korner flag/.claude/notes/` — works in any directory
