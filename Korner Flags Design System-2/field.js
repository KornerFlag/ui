...
(function () {
  "use strict";
  const W = 1200, H = 645;            // svg coordinate space
  const MX = 46, MY = 36;             // pitch margins
  ...
  const awayN = [
    [0.56, 0.40], [0.60, 0.62], [0.50, 0.50], [0.72, 0.30], [0.72, 0.70], [0.86, 0.50],
  ];
  // passing build-up sequence (indices into home)
  const seq = [0, 2, 6, 5, 1, 6, 7, 9, 10, 9, 8, 6, 5, 9];
  const labels = {
    "0-1": "Build from the back", "3-4": "Switch of play",
    "6-7": "Overload right", "8-9": "Third-man run", "10-11": "Final third entry",
  };
  ...
  // mowing stripes
  const stripes = mk("g", { opacity: ".5" });
  const sw = PW / 7;
  for (let i = 0; i < 7; i++) if (i % 2) stripes.appendChild(mk("rect", { x: PX + i * sw, y: PY, width: sw, height: PH, fill: "rgba(255,255,255,.018)" }));
  svg.appendChild(stripes);

  // heat layer (under lines)
  const heat = mk("g", { opacity: "0" });
  ...
  heatBlobs.forEach(([x,y,r,c]) => heat.appendChild(mk("circle", { cx: toX(x), cy: toY(y), r, fill: c, filter: "url(#soft)", opacity: ".55" })));
  svg.appendChild(heat);

  // pitch lines
  const line = (a) => mk("rect", Object.assign({ fill: "none", stroke: "rgba(255,255,255,.16)", "stroke-width": 1.5 }, a));
  const lg = mk("g", {});
  lg.appendChild(line({ x: PX, y: PY, width: PW, height: PH, rx: 4 }));
  lg.appendChild(mk("line", { x1: PX + PW/2, y1: PY, x2: PX + PW/2, y2: PY + PH, stroke: "rgba(255,255,255,.16)", "stroke-width": 1.5 }));
  lg.appendChild(mk("circle", { cx: PX + PW/2, cy: PY + PH/2, r: 64, fill: "none", stroke: "rgba(255,255,255,.16)", "stroke-width": 1.5 }));
  lg.appendChild(mk("circle", { cx: PX + PW/2, cy: PY + PH/2, r: 3, fill: "rgba(255,255,255,.28)" }));
  // boxes
  const boxH = PH * 0.55, box6 = PH * 0.24;
  lg.appendChild(line({ x: PX, y: PY + (PH - boxH)/2, width: 110, height: boxH }));
  lg.appendChild(line({ x: PX, y: PY + (PH - box6)/2, width: 44, height: box6 }));
  lg.appendChild(line({ x: PX + PW - 110, y: PY + (PH - boxH)/2, width: 110, height: boxH }));
  lg.appendChild(line({ x: PX + PW - 44, y: PY + (PH - box6)/2, width: 44, height: box6 }));
  svg.appendChild(lg);
  ...
  // away dots (outline)
  awayN.forEach(([x, y]) => {
    gAway.appendChild(mk("circle", { cx: toX(x), cy: toY(y), r: 9, fill: "rgba(14,24,34,.5)", stroke: "rgba(255,255,255,.4)", "stroke-width": 1.5 }));
  });

  // home dots
  const homeEls = homeN.map(([x, y]) => {
    const g = mk("g", {});
    const halo = mk("circle", { cx: toX(x), cy: toY(y), r: 11, fill: "#2C6AD4", opacity: "0" });
    const dot = mk("circle", { cx: toX(x), cy: toY(y), r: 9.5, fill: "#3F86E0", stroke: "#cfe0f7", "stroke-width": 1.5 });
    g.appendChild(halo); g.appendChild(dot);
    gHome.appendChild(g);
    return { g, dot, halo, bx: toX(x), by: toY(y) };
  });

  // ball
  const ball = mk("circle", { r: 7, fill: "url(#ballG)", stroke: "rgba(14,24,34,.25)", "stroke-width": 1 });
  ...
  function setMode(m) {
    ...
    gHome.style.opacity = dim; gAway.style.opacity = m === "heat" ? ".2" : "1";
    gPass.style.opacity = m === "tracking" ? ".25" : "1";
    gTrail.style.opacity = m === "tracking" ? "1" : ".6";
  }

  // mode buttons
  document.querySelectorAll(".board-mode").forEach(b => b.addEventListener("click", () => { setMode(b.dataset.mode); userMode = performance.now(); }));
  let userMode = 0;
  ...
  function startSeg() {
    ...
    const A = homeEls[fromI], B = homeEls[toI];
    // draw pass line
    const ln = mk("line", { x1: A.cx0 ?? A.bx, y1: A.by, x2: A.bx, y2: A.by, stroke: "rgba(120,180,250,.85)", "stroke-width": 2, "stroke-linecap": "round" });
    const len = Math.hypot(B.bx - A.bx, B.by - A.by);
    ln.setAttribute("x1", A.bx); ln.setAttribute("y1", A.by);
    ln.setAttribute("stroke-dasharray", len); ln.setAttribute("stroke-dashoffset", len);
    gPass.appendChild(ln);
    passLines.push({ el: ln, born: performance.now(), len, x2: B.bx, y2: B.by, A });
    // label
  ...
  function frame(now) {
    ...
    homeEls.forEach((p, i) => {
      ...
      p.dot.setAttribute("cx", p.bx + ox); p.dot.setAttribute("cy", p.by + oy);
      p.halo.setAttribute("cx", p.bx + ox); p.halo.setAttribute("cy", p.by + oy);
    });
    ...
    if (!reduce) {
      ...
      if (segT >= 1) {
        ...
        requestAnimationFrame(() => { B.halo.style.transition = "opacity .7s"; B.halo.setAttribute("opacity", "0"); });
        passCount++; if (elPasses) elPasses.textContent = passCount;
        segIdx++; segT = 0; cur = startSeg();
      }
      ...
      if (lp) { lp.el.setAttribute("x2", bx); lp.el.setAttribute("y2", by); lp.el.setAttribute("stroke-dashoffset", lp.len * (1 - e)); }

      // ball trail
      trail.push([bx, by]); if (trail.length > 16) trail.shift();
      ...
      for (let i = passLines.length - 2; i >= 0; i--) {
        ...
        if (passLines.length > 7 && i === 0) { gPass.removeChild(pl.el); passLines.shift(); }
      }
    } else {
      ball.setAttribute("cx", cur.B.bx); ball.setAttribute("cy", cur.B.by);
    }
    ...
    trail.forEach((p, i) => {
      ...
      gTrail.appendChild(mk("circle", { cx: p[0], cy: p[1], r: 2 + a * 3.5, fill: `rgba(120,180,250,${a * 0.5})` }));
    });

    // timeline + clock
    const loopT = ((now - t0) / 1000 % 24) / 24;
    if (elTimeFill) elTimeFill.style.width = (8 + loopT * 84) + "%";
    if (elTimeNow) { const s = matchStart + Math.floor(loopT * 220); elTimeNow.textContent = `${String(Math.floor(s/60)).padStart(2,"0")}:${String(s%60).padStart(2,"0")}`; }

    // gentle possession sway
    if (elPossA) { const p = 54 + Math.round(Math.sin(t * 0.2) * 2); elPossA.style.width = p + "%"; elPossB.style.width = (100 - p) + "%"; }
  ...
  setMode("passing");
  if (reduce) { // draw a few static passes for a rich still frame
    [[0,2],[2,6],[6,9],[9,10]].forEach(([a,b]) => {
      gPass.appendChild(mk("line", { x1: homeEls[a].bx, y1: homeEls[a].by, x2: homeEls[b].bx, y2: homeEls[b].by, stroke: "rgba(120,180,250,.55)", "stroke-width": 2, "stroke-linecap": "round" }));
    });
  }
  requestAnimationFrame(frame);
